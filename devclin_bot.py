...
MPESA_RECEIVER_NUMBER = "0743810633"          # Phone number for Send Money
...
def parse_mpesa_message(msg: str, expected_amount: float):
    ...
    msg_upper = msg.upper()
    name_upper = receiver_name.upper()

    # Check receiver name
    if name_upper not in msg_upper:
        return False, f"❌ Receiver name not found. Expected *{receiver_name}* in the M-Pesa message."

    # Check receiver phone number (handle formats: 0743810633 / 254743810633 / +254743810633)
    number_clean = receiver_number.replace("+", "").replace(" ", "")
    # Also check 254 format
    ...
    # Extract amount — match patterns like "Ksh500.00" or "Ksh 500.00"
    ...
    # Extract receipt/transaction code (e.g. ABC123DE or QHJ1234XY)
