...
async def search_youtube(query: str):
    ...
    if YOUTUBE_API_KEY:
        ...
        # Fallback: use invidious public API (privacy-friendly YouTube proxy)
        try:
...
async def download_media_cobalt(url: str) -> dict:
    ...
    Returns: {"url": direct_link, "filename": ..., "quality": ...} or {"error": msg}
    """
...
async def ask_claude(user_message: str, history: list = None) -> str:
    ...
    if not ANTHROPIC_API_KEY:
        return (
            ...
            "Ask the admin to set the `ANTHROPIC_API_KEY` environment variable to enable this feature."
        )
...
async def payment_initiate(query, context, prod_id: str):
    ...
    user_states[user_id] = AWAIT_MPESA_MSG
...
async def prompt_music_search(query, context):
    user_id = query.from_user.id
    user_states[user_id] = AWAIT_MUSIC_SEARCH
...
async def handle_music_search(update: Update, context: ContextTypes.DEFAULT_TYPE, query_text: str):
    """Search iTunes and present results."""
    user = update.effective_user
    user_states.pop(user.id, None)
    ...
    text = f"🎵 *Results for:* _{escape_md(query_text)}_\n\n"
    buttons = []
    for i, r in enumerate(results[:5]):
        artist  = r.get("artistName", "Unknown Artist")
        track   = r.get("trackName", "Unknown Track")
        preview = r.get("previewUrl", "")    # 30-sec Apple preview MP3
        itunes  = r.get("trackViewUrl", "")  # Full link on Apple Music

        text += f"{i+1}. 🎵 *{escape_md(track)}*\n   👤 {escape_md(artist)}\n"

        row = []
        if preview:
            row.append(InlineKeyboardButton(f"▶ Preview #{i+1}", url=preview))
        if itunes:
            row.append(InlineKeyboardButton("🍎 Apple Music", url=itunes))
        if row:
            buttons.append(row)
...
async def prompt_ai_chat(query, context):
    user_id = query.from_user.id
    user_states[user_id] = AWAIT_AI_CHAT
    text = (
        "🤖 *AI Chat Active*\n\n"
    ...
    if state == AWAIT_MPESA_MSG or user.id in pending_payments:
        ...
        if ok:
            ...
            admin_msg = (
                ...
                f"🆔 ID: `{user.id}`\n\n"
                f"📦 Product: *{prod['name']}*\n"
                f"💰 Amount Paid: *KSh {result['amount']:,.0f}*\n"
                f"🧾 Receipt: `{result.get('receipt', 'N/A')}`\n"
                f"📅 Date: {result['date']}\n\n"
                f"M-Pesa msg:\n`{escape_md(text[:200])}`\n\n"
