...
async def play_music_track(query, context, owner_id: int, track_index: int):
    ...
    # Try yt-dlp first for full song, fall back to Deezer 30s preview
    audio_url  = None
    duration   = None
    used_ytdlp = False
    ...
    try:
        ...
        search_term = f"{track_name} {artist_name} audio"
        proc = subprocess.run(
            [
                "yt-dlp",
        ...
        if len(lines) >= 1 and lines[0].startswith("http"):
            audio_url  = lines[0]
            used_ytdlp = True
            # Parse duration if returned (format mm:ss or hh:mm:ss)
            if len(lines) >= 2:
                parts = lines[1].split(":")
                try:
                    if len(parts) == 2:
        ...
        logger.info(f"yt-dlp not available or failed: {e}")

    # Fall back to Deezer 30s preview
    if not audio_url and preview_url:
        audio_url = preview_url
        duration  = 30  # Deezer previews are always 30 seconds

    if not audio_url:
        await query.message.reply_text(
            f"❌ Could not load audio for *{_html.escape(track_name)}*.\n"
            f"Try another track.",
            parse_mode="HTML"
        )
        return

    # Build next/prev controls
    total   = len(results)
    nav_row = []
    if track_index > 0:
    ...
    quality_note = "" if used_ytdlp else "\n_30s preview · Install yt-dlp for full songs_"
    ...
    if used_ytdlp:
        ...
        caption += f"🎧 30s Preview"
    caption += f"\n\n<i>{BOT_NAME} | {COMPANY}</i>"
    ...
    try:
        ...
        # Fallback: send as voice note if audio fails
        try:
            if preview_url:
                await query.message.reply_audio(
                    audio=preview_url,
                ...
                await query.message.reply_text(
                    f"❌ Could not stream this track. Try another.",
                    reply_markup=InlineKeyboardMarkup(buttons)
                )
        except Exception as e2:
            logger.error(f"Audio fallback also failed: {e2}")
            await query.message.reply_text(
                f"❌ Audio stream failed for *{track_name}*. Try another track.",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup(buttons)
            )
