...
def init_db():
    ...
    c.execute("INSERT OR IGNORE INTO settings VALUES ('banner_shop', '')")
    c.execute("INSERT OR IGNORE INTO settings VALUES ('banner_downloader', '')")
    c.execute("INSERT OR IGNORE INTO settings VALUES ('banner_ai', '')")
    c.execute("INSERT OR IGNORE INTO settings VALUES ('banner_account', '')")
    c.execute("INSERT OR IGNORE INTO settings VALUES ('banner_contact', '')")
    c.execute("INSERT OR IGNORE INTO settings VALUES ('banner_payment', '')")
    ...
    c.execute("INSERT OR IGNORE INTO settings VALUES ('banner_services', '')")
    c.execute("INSERT OR IGNORE INTO settings VALUES ('banner_links', '')")
    c.execute("INSERT OR IGNORE INTO settings VALUES ('banner_about', '')")
    c.execute("INSERT OR IGNORE INTO settings VALUES ('banner_group', '')")
    c.execute("INSERT OR IGNORE INTO settings VALUES ('banner_rate', '')")
    c.execute("INSERT OR IGNORE INTO settings VALUES ('sms_verify_enabled', '0')")

    # ── Real M-Pesa SMS inbox (forwarded from phone) ──
    c.execute('''CREATE TABLE IF NOT EXISTS received_mpesa (
...
async def send_cyber_footer(update_or_query, context, caption, keyboard, section=None):
    """Always sends a NEW message so the main menu stays visible."""
    _s = get_settings()
    default = _s.get("bot_banner_image", "") or CYBER_IMAGE
    # Per-section banner lookup
    section_map = {
        "shop":        _s.get("banner_shop", "") or default,
        "downloader":  _s.get("banner_downloader", "") or default,
        "ai":          _s.get("banner_ai", "") or default,
        "account":     _s.get("banner_account", "") or default,
        "contact":     _s.get("banner_contact", "") or default,
        "payment":     _s.get("banner_payment", "") or default,
        "services":    _s.get("banner_services", "") or default,
        "links":       _s.get("banner_links", "") or default,
        "about":       _s.get("banner_about", "") or default,
        "group":       _s.get("banner_group", "") or default,
        "rate":        _s.get("banner_rate", "") or default,
    }
...
def parse_mpesa_message(msg: str, expected_amount: float):
    ...
    - Transaction code exists in real received SMS inbox (if enabled)
    ...
    # ── SMS Inbox Verification (if enabled) ────────────────────────────
    # Only runs if admin has enabled it and SMS forwarder is set up
    settings = get_settings()
    if settings.get("sms_verify_enabled", "0") == "1" and receipt != "N/A":
        try:
            conn = sqlite3.connect(DB_PATH)
            row = conn.execute(
                "SELECT * FROM received_mpesa WHERE txn_code=?", (receipt,)
            ).fetchone()
            conn.close()
            if not row:
                return False, (
                    f"❌ *Transaction Not Found*\n\n"
                    f"Code `{receipt}` was not found in our M-Pesa inbox.\n\n"
                    f"This could mean:\n"
                    f"• The SMS is fake or edited\n"
                    f"• Payment not yet received\n\n"
                    f"If you just paid, wait 1 minute and try again."
                )
        except Exception as e:
            pass  # If DB check fails, fall through to allow payment
...
async def search_youtube(query: str):
    ...
    if yt_key:
        ...
        # Try multiple public Invidious instances in order — first one that responds wins
        invidious_instances = [
        ...
        for instance in invidious_instances:
            try:
                inv_url = f"{instance}/api/v1/search?{params}"
                def _fetch2(u=inv_url):
                    req = urllib.request.Request(u, headers={"User-Agent": "DevClinBot/1.0"})
                ...
                continue
        if not results:
            logger.error("All Invidious instances failed for YouTube search.")
    return results

# ══════════════════════════════════════════════
#   DOWNLOADER  (via RapidAPI — Social Download All In One)
#   API: https://social-download-all-in-one.p.rapidapi.com
#   Supports: TikTok, YouTube, Facebook, Instagram, Twitter, Spotify, etc.
# ══════════════════════════════════════════════
RAPIDAPI_DOWNLOADER_KEY = os.environ.get("RAPIDAPI_KEY", "")  # Set via env var or Admin Panel → Settings

async def download_media_cobalt(url: str) -> dict:
    """
    Use RapidAPI 'Social Download All In One' to resolve a download link.
    Supports: TikTok, YouTube, Facebook, Instagram, Twitter, Pinterest, Spotify.
    Returns: {"url": direct_link, "filename": ..., "type": "video"|"audio"} or {"error": msg}
    """
    import asyncio

    # Allow admin to override the key via settings DB
    live_key = get_api_setting("downloader_api_key", "RAPIDAPI_KEY") or RAPIDAPI_DOWNLOADER_KEY
    if not live_key:
        return {"error": "⚙️ Downloader not configured. Admin must add a RapidAPI key in Admin Panel → Settings (downloader_api_key), or set the RAPIDAPI_KEY env var. Get a free key at rapidapi.com"}

    api_url = "https://social-download-all-in-one.p.rapidapi.com/v1/social/autolink"
    payload = _json.dumps({"url": url}).encode()
    headers = {
    ...
    try:
        ...
        if data.get("error"):
            return {"error": data.get("message", "Download failed. Check the link.")}

        medias = data.get("medias", [])
        if not medias:
            return {"error": "No downloadable media found in that link."}

        # Prefer HD no-watermark video first, then any video, then audio
        chosen = None
        for quality in ("hd_no_watermark", "no_watermark", "hd", "sd", "watermark"):
            for m in medias:
                if m.get("quality") == quality and m.get("type") == "video":
                    chosen = m
                    break
            if chosen:
                break
        ...
        if not chosen:
            # Fall back to audio or first available
            for m in medias:
                if m.get("type") == "audio":
                    chosen = m
                    break
        ...
        ext = chosen.get("extension", "mp4")
        media_type = chosen.get("type", "video")
        title = data.get("title", "media")
        filename = f"{title[:40]}.{ext}"

        return {
            "url":      chosen.get("url", ""),
            "filename": filename,
            "type":     media_type,   # "video" or "audio"
            "title":    title,
            "source":   data.get("source", ""),
        }

    except Exception as e:
        logger.error(f"RapidAPI Downloader error: {e}")
...
async def show_shop(query, context):
    ...
    await send_cyber_footer(query, context, "🛍 *Shop — Choose a Category*", buttons, section="shop")
...
async def show_category(query, context, cat: str):
    ...
    buttons.append(back_home_row())
    buttons.append(me_row())
    await send_cyber_footer(query, context, text, buttons, section="shop")
...
async def payment_initiate(query, context, prod_id: str):
    ...
    await send_cyber_footer(query, context, text, buttons, section="payment")
...
async def show_downloader_menu(query, context):
    ...
    await send_cyber_footer(query, context, text, buttons, section="downloader")
...
async def handle_downloader_link(update: Update, context: ContextTypes.DEFAULT_TYPE, url: str):
    """Download via RapidAPI Social Download All In One and send the file."""
    user = update.effective_user
    ...
    media_type = result.get("type", "video")   # "video" or "audio" from RapidAPI

    try:
        # Use the media type returned by the API first; fall back to extension sniffing
        is_audio = (media_type == "audio") or fname.endswith((".mp3", ".m4a", ".ogg", ".aac"))
        if is_audio:
...
async def show_ai_menu(query, context):
    ...
    await send_cyber_footer(query, context, text, buttons, section="ai")
...
async def show_services(query, context):
    ...
    await send_cyber_footer(query, context, text, buttons, section="services")
async def show_links(query, context):
    ...
    await send_cyber_footer(query, context, text, buttons, section="links")
...
async def show_about(query, context):
    ...
    await send_cyber_footer(query, context, text, buttons, section="about")
...
async def show_contact(query, context):
    ...
    await send_cyber_footer(query, context, text, buttons, section="contact")
...
async def show_group_chat(query, context):
    ...
    await send_cyber_footer(query, context, text, buttons, section="group")
...
async def show_bot_rating(query, context):
    ...
    await send_cyber_footer(query, context, text, buttons, section="rate")
    user_id = query.from_user.id
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Priority states — checked BEFORE M-Pesa ────────────────────────
    ...
    # ── M-Pesa verification ────────────────────────────────────────────
    # Only enter payment flow when state is explicitly AWAIT_MPESA_MSG.
    # Do NOT rely on pending_payments alone — that caused AI/downloader
    # messages to be mis-routed into payment verification.
    if state == AWAIT_MPESA_MSG:
        ...
        if ok:
            ...
            if prod["link"]:
                ...
                # 3. Send download link as the LAST message so user sees it clearly
                try:
                    await update.message.reply_text(
                    ...
                    logger.error(f"Download link send error: {e}")
                    # Fallback — try sending just the raw link
                    try:
                        await update.message.reply_text(one_time_link)
                    except:
                        pass
            else:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Downloader ─────────────────────────────────────────────────────
        ...
        await prompt_downloader(query, context, "instagram", AWAIT_DL_LINK)
    elif data == "dl_twitter":
        await prompt_downloader(query, context, "twitter", AWAIT_DL_LINK)
