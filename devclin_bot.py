...
def generate_download_token(user_id: int, product_id: str, file_url: str) -> str:
    """
    Create a one-time download token with NO expiry.
    Link works once only — once clicked it is burned.
    ...
    try:
        ...
        conn.execute(
            ...
            (token, user_id, product_id, file_url, "9999-12-31T23:59:59", datetime.now().isoformat())
        )
        conn.commit()
        conn.close()
    except Exception as e:
...
def add_loyalty_points(user_id: int, points: int, reason: str = ""):
    """Add loyalty points to a user and log it."""
    try:
        conn = get_db()
        conn.execute(
...
def parse_mpesa_message(msg: str, expected_amount: float):
    ...
    # ── Transaction code reuse check ────────────────────────────────────
    # If this code was already used, warn user and block
    if receipt != "N/A":
        try:
            conn = sqlite3.connect(DB_PATH)
            row = conn.execute(
                "SELECT user_id FROM used_transactions WHERE txn_id=?", (receipt,)
            ).fetchone()
            conn.close()
            if row:
                return False, (
                    ...
                    "⚠️ *Warning: If you continue submitting used or fake payment "
                    "confirmations, you will be suspended from the marketplace.*\n\n"
...
#   API: cobalt.tools — FREE, no key needed
#   Supports: YouTube, TikTok, Instagram, Twitter, Facebook, SoundCloud, Spotify
# ══════════════════════════════════════════════

async def download_media_cobalt(url: str) -> dict:
    """
    Use cobalt.tools API — completely FREE, no key needed.
    Supports: YouTube, TikTok, Instagram, Twitter, Facebook, Spotify, SoundCloud.
    ...
    try:
        ...
        if status == "error":
            ...
            return {"error": f"❌ Could not download: {code}. Check the link and try again."}

        if status in ("redirect", "tunnel"):
            dl_url  = data.get("url", "")
            filename = data.get("filename", "media.mp4")
            media_type = "audio" if filename.endswith((".mp3", ".ogg", ".m4a")) else "video"
            return {"url": dl_url, "filename": filename, "type": media_type, "title": filename.rsplit(".", 1)[0]}

        if status == "picker":
            picks = data.get("picker", [])
            for p in picks:
                if p.get("type") == "video":
                    return {"url": p.get("url", ""), "filename": "video.mp4", "type": "video", "title": "media"}
            if picks:
                return {"url": picks[0].get("url", ""), "filename": "media.mp4", "type": "video", "title": "media"}

        return {"error": "❌ Could not extract download link. Check the URL and try again."}

    except Exception as e:
        logger.error(f"Cobalt downloader error: {e}")
        return {"error": "⚠️ Download service unavailable. Try again later."}
...
async def show_shop(query, context):
    ...
    buttons.append([InlineKeyboardButton("🔍 Search Products", switch_inline_query_current_chat="")])
...
async def show_links(query, context):
    ...
    buttons = [
        ...
        [InlineKeyboardButton("📞 Call Us", url=CALL_NUM)],
...
async def inline_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for p in matches[:10]:
        sale_info = ""
        if p.get("sale_price") and float(p.get("sale_price", 0)) > 0:
            sale_info = f"🔥 Sale: KSh {float(p['sale_price']):,.0f} (was {p['price']})"
        else:
            sale_info = f"💰 Price: {p['price']}"
        ...
        desc_text = (
            ...
            f"2️⃣ Go to Shop → find *{p['name']}*\n"
            f"3️⃣ Send M-Pesa to {recv_num} ({recv_name})\n"
            f"4️⃣ Paste confirmation SMS in bot"
        )
        results.append(
            InlineQueryResultArticle(
                id=str(_uuid.uuid4()),
                title=f"{p['icon']} {p['name']} — {p['price']}",
                description=p["desc"][:80] if p["desc"] else p["name"],
