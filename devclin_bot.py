...
async def search_music_itunes(query: str):
    """Search music using multiple free APIs with fallbacks."""
    import asyncio
    ...
    # ── 1. Try Deezer (most reliable, no key needed) ──────────────────
    try:
        url = f"https://api.deezer.com/search?q={urllib.parse.quote(query)}&limit=8"
        data = await loop.run_in_executor(None, lambda: _fetch(url))
        results = data.get("data", [])
        if results:
            logger.info(f"Music search: Deezer returned {len(results)} results for '{query}'")
            return [{
            ...
            errors.append("Deezer: 0 results")
    except Exception as e:
        logger.error(f"Music search Deezer error: {e}")
        errors.append(f"Deezer: {type(e).__name__}")
    ...
    try:
        ...
        if results:
            logger.info(f"Music search: iTunes returned {len(results)} results for '{query}'")
            for r in results:
                r["_source"] = "itunes"
            return results
        else:
            errors.append("iTunes: 0 results")
    except Exception as e:
        logger.error(f"Music search iTunes error: {e}")
        errors.append(f"iTunes: {type(e).__name__}")

    # ── 3. Try MusicBrainz (always free, no key) ─────────────────────
    try:
        ...
        data = await loop.run_in_executor(None, lambda: _fetch(url, {
            "User-Agent": "DevClinBot/1.0 ( bot@devclin.com )",
            "Accept": "application/json"
        }))
        recordings = data.get("recordings", [])
        if recordings:
            logger.info(f"Music search: MusicBrainz returned {len(recordings)} results for '{query}'")
            normalized = []
            for r in recordings:
                ...
                normalized.append({
                    ...
                    "artistName":   artist,
                    "previewUrl":   "",
                    "trackViewUrl": f"https://musicbrainz.org/recording/{r.get('id','')}",
                    "_source": "musicbrainz",
                })
            return normalized
        else:
            errors.append("MusicBrainz: 0 results")
    except Exception as e:
        logger.error(f"Music search MusicBrainz error: {e}")
        errors.append(f"MusicBrainz: {type(e).__name__}")

    logger.warning(f"Music search failed all APIs for '{query}'. Errors: {errors}")
    return [{"_error": True, "_details": " | ".join(errors)}]
...
async def handle_music_search(update: Update, context: ContextTypes.DEFAULT_TYPE, query_text: str):
    """Search iTunes/Deezer and present results as inline play list."""
    user = update.effective_user
    clear_user_state(user.id)
    ...
    if not results:
        ...
        await update.message.reply_text(
            "😔 No results found. Try a different song name or artist.",
            reply_markup=InlineKeyboardMarkup(buttons)
        )
        return

    # Handle API error case
    if results[0].get("_error"):
        details = results[0].get("_details", "Unknown error")
        buttons = [
            [InlineKeyboardButton("🔍 Try Again", callback_data="music_search")],
            back_home_row(),
        ]
        await update.message.reply_text(
            f"⚠️ <b>Music search temporarily unavailable.</b>\n\n"
            f"All music APIs failed to respond.\n\n"
            f"<i>Details: {details}</i>\n\n"
    ...
    # Store results in user_data so Next/Play buttons can access them
    valid_results = [r for r in results[:8] if r.get("previewUrl") or r.get("trackViewUrl")]
    if not valid_results:
        valid_results = results[:8]

    context.user_data[f"music_results_{user.id}"] = valid_results
    context.user_data[f"music_query_{user.id}"] = query_text

    import html as _html
    src = results[0].get("_source", "itunes")
    source_label = {"deezer": "Deezer", "musicbrainz": "MusicBrainz"}.get(src, "iTunes")
    ...
    # Build play buttons — one per result
    buttons = []
    for i in range(min(5, len(valid_results))):
        r = valid_results[i]
        track = r.get("trackName", f"Track {i+1}")[:25]
        buttons.append([InlineKeyboardButton(
            f"▶ Play #{i+1} — {track}",
            callback_data=f"music_play_{user.id}_{i}"
        )])
...
async def play_music_track(query, context, owner_id: int, track_index: int):
    """Fetch and send a music track as a Telegram audio message with full player."""
    import html as _html

    results = context.user_data.get(f"music_results_{owner_id}", [])
    if not results or track_index >= len(results):
        await query.edit_message_caption(
            caption="❌ Track no longer available. Please search again.",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🔍 Search Again", callback_data="music_search")
            ]])
        )
        return

    r = results[track_index]
    track_name  = r.get("trackName", "Unknown Track")
    artist_name = r.get("artistName", "Unknown Artist")
    preview_url = r.get("previewUrl", "")
    src         = r.get("_source", "itunes")

    await query.answer(f"⏳ Loading: {track_name[:30]}...")

    # Build next/prev controls (defined early so error messages can use buttons)
    total   = len(results)
    nav_row = []
    if track_index > 0:
        nav_row.append(InlineKeyboardButton(
            "⏮ Prev", callback_data=f"music_play_{owner_id}_{track_index - 1}"
        ))
    nav_row.append(InlineKeyboardButton(
        f"🎵 {track_index + 1}/{total}", callback_data="noop"
    ))
    if track_index < total - 1:
        nav_row.append(InlineKeyboardButton(
            "⏭ Next", callback_data=f"music_play_{owner_id}_{track_index + 1}"
        ))

    buttons = [
        nav_row,
        [InlineKeyboardButton("🔍 New Search", callback_data="music_search"),
         InlineKeyboardButton("🏠 Home",       callback_data="home")],
    ]

    # ── Try yt-dlp: download full audio to a temp file ────────────────
    import subprocess, tempfile, os as _os
    audio_file  = None   # path to downloaded temp file
    duration    = None
    used_ytdlp  = False

    try:
        search_term = f"{track_name} {artist_name} official audio"
        with tempfile.TemporaryDirectory() as tmpdir:
            out_template = _os.path.join(tmpdir, "audio.%(ext)s")
            proc = subprocess.run(
                [
                    "yt-dlp",
                    ...
                    "--quiet",
                    "--no-warnings",
                    "--print", "duration",   # print duration (seconds) to stdout
                    "-o", out_template,
                ],
            ...
            downloaded = [
                _os.path.join(tmpdir, f)
                for f in _os.listdir(tmpdir)
                if f.startswith("audio")
            ]
            if downloaded:
                audio_file = downloaded[0]
                # Parse duration from stdout (yt-dlp --print duration outputs seconds)
                dur_line = proc.stdout.strip().splitlines()
                for line in dur_line:
                    try:
                        duration = int(float(line.strip()))
                        break
                    except ValueError:
                        pass
                used_ytdlp = True
                ...
                caption = (
                    f"🎵 <b>{_html.escape(track_name)}</b>\n"
                    f"👤 {_html.escape(artist_name)}\n"
                    f"🎧 Full song\n\n"
                    f"<i>{BOT_NAME} | {COMPANY}</i>"
                )
    ...
    # ── Fallback: Deezer 30s preview (URL-based) ──────────────────────
    if preview_url:
        caption = (
            f"🎵 <b>{_html.escape(track_name)}</b>\n"
            f"👤 {_html.escape(artist_name)}\n"
            f"🎧 30s Preview <i>(install yt-dlp on server for full songs)</i>\n\n"
            f"<i>{BOT_NAME} | {COMPANY}</i>"
        )
        try:
            await query.message.reply_audio(
            ...
            logger.error(f"Deezer preview send error: {e}")

    # ── Total failure ─────────────────────────────────────────────────
    await query.message.reply_text(
        f"❌ Could not load audio for <b>{_html.escape(track_name)}</b>.\n"
        f"Try another track.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(buttons)
    )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Music ──────────────────────────────────────────────────────────
        ...
        # format: music_play_{owner_id}_{track_index}
        parts = data.split("_")
        # ['music', 'play', owner_id, track_index]
        try:
            owner_id    = int(parts[2])
            track_index = int(parts[3])
            await play_music_track(query, context, owner_id, track_index)
        except (IndexError, ValueError) as e:
            logger.error(f"music_play parse error: {e} — data={data}")
            await query.answer("❌ Error loading track.")
    elif data == "noop":
        await query.answer()
