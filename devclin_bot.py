...
async def handle_music_search(update: Update, context: ContextTypes.DEFAULT_TYPE, query_text: str):
    ...
    # Handle API error case — show helpful message instead of silent failure
    if results[0].get("_error"):
        ...
        await update.message.reply_text(
            f"⚠️ <b>Music search is temporarily unavailable.</b>\n\n"
            f"All music APIs failed to respond. This usually means the server's internet connection is restricted.\n\n"
    ...
    for i, r in enumerate(results[:5]):
        ...
        if store:
            if source_label == "iTunes":
                label = "🍎 Apple Music"
            elif source_label == "Deezer":
                label = "🎧 Deezer"
            else:
                label = "🎵 MusicBrainz"
