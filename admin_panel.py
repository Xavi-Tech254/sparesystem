...
def _notify_new_product_admin(prod):
    banner = get_setting("bot_banner_url") or "https://i.postimg.cc/CLHFDLbK/Gemini-Generated-Image-avf6o5avf6o5avf6.png"
    conn = get_db()
    users = conn.execute("SELECT user_id FROM users").fetchall()
    conn.close()
    msg = (
...
def _notify_price_drop_admin(prod, orig_price, new_price):
    banner = get_setting("bot_banner_url") or "https://i.postimg.cc/CLHFDLbK/Gemini-Generated-Image-avf6o5avf6o5avf6.png"
    conn = get_db()
    users = conn.execute("SELECT user_id FROM users").fetchall()
    conn.close()
...
<div class="sidebar">
  ...
  <div class="nav">
    ...
    <a href="/admin/ratings" class="{{ 'active' if page=='ratings' }}">⭐ Product Reviews</a>
    <a href="/admin/bot-ratings" class="{{ 'active' if page=='bot_ratings' }}">🌟 Bot Reviews</a>
...
def products():
    ...
    if request.method == "POST":
        ...
        if action == "add":
            ...
            drop_price = float(request.form.get("drop_price", 0) or 0)
            notify_users = request.form.get("notify_users") == "on"
            try:
                conn.execute(
                    "INSERT INTO products VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                    (pid, name, cat, price, price_val, ptype, desc, link, icon, 1, image_url, original_price, drop_price)
                )
                conn.commit()
                flash(f"Product '{name}' added!", "success")
                # Broadcast notification via Telegram if checkbox ticked
                if notify_users:
                    prod = {"id": pid, "name": name, "icon": icon, "price": price,
                            "type": ptype, "desc": desc, "image_url": image_url}
                    _notify_new_product_admin(prod)
                # Check for price drop
                if original_price > 0 and drop_price > 0 and drop_price < original_price:
                    prod = {"id": pid, "name": name, "icon": icon, "price": price,
                            "type": ptype, "desc": desc, "image_url": image_url}
                    _notify_price_drop_admin(prod, original_price, drop_price)
    ...
    <div class="card">
      ...
      <form method="POST">
        ...
        <div class="form-row">
          <div class="form-group"><label>PRICE LABEL (e.g. KSh 500)</label><input name="price" placeholder="KSh 500" required></div>
        ...
        <div class="form-row">
          <div class="form-group"><label>ORIGINAL PRICE (before drop)</label><input name="original_price" type="number" placeholder="0" value="0"></div>
          <div class="form-group"><label>PRICE AFTER DROP (0 = no drop)</label><input name="drop_price" type="number" placeholder="0" value="0"></div>
        </div>
        <div class="form-row">
        ...
        <div class="form-group"><label>PRODUCT IMAGE URL (optional — shown in bot)</label><input name="image_url" placeholder="https://...image.jpg"></div>
        ...
        <div class="form-group" style="display:flex;align-items:center;gap:10px">
          <input type="checkbox" name="notify_users" id="notify_users" style="width:auto">
          <label for="notify_users" style="margin:0;font-size:13px;color:var(--text)">📢 Notify all users about this new product</label>
        </div>
...
def bot_settings():
    ...
    if request.method == "POST":
        ...
        if action == "change_password":
            ...
            conn.commit()
            flash("API keys saved successfully! They are now active in the bot.", "success")
        elif action == "branding":
            logo_url   = request.form.get("admin_logo_url", "").strip()
            banner_url = request.form.get("bot_banner_url", "").strip()
            group_link = request.form.get("group_chat_link", "").strip()
            conn.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('admin_logo_url', ?)", (logo_url,))
            conn.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('bot_banner_url', ?)", (banner_url,))
            conn.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('group_chat_link', ?)", (group_link,))
            conn.commit()
    ...
    admin_logo   = settings.get("admin_logo_url", "")
    bot_banner   = settings.get("bot_banner_url", "")
    ...
    <div class="card">
      ...
      <p style="font-size:12px;color:var(--muted);margin-bottom:16px">
        Set your admin panel logo, bot banner image, and group chat link.
      </p>
      <form method="POST">
        <input type="hidden" name="action" value="branding">
        <div class="form-group">
          <label>🖼 ADMIN PANEL LOGO URL</label>
          <input name="admin_logo_url" value="{admin_logo}" placeholder="https://...logo.png">
          {'<img src="' + admin_logo + '" style="height:50px;margin-top:8px;border-radius:8px">' if admin_logo else ''}
          <p style="font-size:11px;color:var(--muted);margin-top:4px">Paste a direct image URL. Shown on the admin sidebar logo area.</p>
        </div>
        <div class="form-group">
          <label>🖼 BOT BANNER IMAGE URL (shown in all bot messages)</label>
          <input name="bot_banner_url" value="{bot_banner}" placeholder="https://...banner.jpg">
          {'<img src="' + bot_banner + '" style="height:80px;margin-top:8px;border-radius:8px">' if bot_banner else ''}
          <p style="font-size:11px;color:var(--muted);margin-top:4px">This image appears as the hero image for all bot messages.</p>
        </div>
        <div class="form-group">
          <label>💬 GROUP CHAT LINK (for the Group Chat menu button)</label>
          <input name="group_chat_link" value="{group_link}" placeholder="https://t.me/yourgroupname">
          <p style="font-size:11px;color:var(--muted);margin-top:4px">Users will be directed here when they tap the Group Chat button.</p>
        </div>
    ...
    <div class="card">
      ...
      <form method="POST">
        ...
        <div class="form-group">
          ...
          <input name="ai_api_key" type="password" placeholder="{'Currently set ✅ — paste to update' if ai_api_key else 'sk-ant-api03-...'}" value="">
          {'<p style="font-size:11px;color:var(--accent);margin-top:4px">✅ Key is set: ' + mask_key(ai_api_key) + '</p>' if ai_api_key else '<p style="font-size:11px;color:var(--danger);margin-top:4px">❌ Not set — AI Assistant will not work</p>'}
        ...
        <div class="form-group">
          ...
          <input name="downloader_api_key" type="password" placeholder="{'Currently set ✅ — paste to update' if dl_api_key else 'AIza...'}" value="">
          {'<p style="font-size:11px;color:var(--accent);margin-top:4px">✅ Key is set: ' + mask_key(dl_api_key) + '</p>' if dl_api_key else '<p style="font-size:11px;color:var(--muted);margin-top:4px">Optional — without it, Invidious (free) is used for YouTube search</p>'}
...
def product_ratings():
    ...
    try:
        rows = conn.execute(
            "SELECT * FROM ratings ORDER BY id DESC LIMIT 100"
        ).fetchall()
    except:
        rows = []
    conn.close()
    ...
    for r in rows:
        ...
        <tr>
          ...
          <td>{stars(r['rating'])} ({r['rating']}/5)</td>
          <td style="max-width:300px">{r['review']}</td>
          <td style="font-family:'Space Mono',monospace;font-size:11px;color:var(--muted)">{str(r['created_at'])[:16]}</td>
        </tr>"""

    content = f"""
    <div class="page-title">Product Reviews</div>
    <div class="page-sub">{len(rows)} reviews received</div>
    <div class="card">
      <table>
        <tr><th>USER</th><th>PRODUCT</th><th>RATING</th><th>REVIEW</th><th>DATE</th></tr>
        {table_rows if table_rows else '<tr><td colspan="5" style="text-align:center;padding:24px;color:var(--muted)">No reviews yet</td></tr>'}
      </table>
    </div>"""
    return render_page(content, "ratings")

@app.route("/admin/bot-ratings")
@login_required
def bot_ratings():
    conn = get_db()
    try:
        rows = conn.execute(
            "SELECT * FROM bot_ratings ORDER BY id DESC LIMIT 100"
        ).fetchall()
    except:
        rows = []
    conn.close()
    ...
    for r in rows:
        ...
        <tr>
          <td>{r['full_name']}<br><span style="font-size:11px;color:var(--muted)">@{r['username'] or 'N/A'}</span></td>
          <td>{stars(r['rating'])} ({r['rating']}/5)</td>
          <td style="max-width:400px">{r['review']}</td>
          <td style="font-family:'Space Mono',monospace;font-size:11px;color:var(--muted)">{str(r['created_at'])[:16]}</td>
        </tr>"""

    content = f"""
    <div class="page-title">Bot Reviews</div>
    <div class="page-sub">{len(rows)} overall reviews</div>
    <div class="card">
      <table>
        <tr><th>USER</th><th>RATING</th><th>REVIEW</th><th>DATE</th></tr>
        {table_rows if table_rows else '<tr><td colspan="4" style="text-align:center;padding:24px;color:var(--muted)">No reviews yet</td></tr>'}
      </table>
    </div>"""
    return render_page(content, "bot_ratings")
