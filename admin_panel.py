...
def bot_settings():
    ...
    <div class="page-sub">Manage payment details and admin credentials</div>

    <div class="card">
      <div class="card-title">📱 M-Pesa Payment Settings</div>
      <p style="font-size:12px;color:var(--muted);margin-bottom:16px">These details are shown to customers and used to verify payments.</p>
      <form method="POST">
        <input type="hidden" name="action" value="mpesa_settings">
        <div class="form-row">
          <div class="form-group">
            <label>RECEIVER NAME (must match M-Pesa exactly)</label>
            <input name="mpesa_name" value="{mpesa_name}" required placeholder="Clinton Oduor">
          </div>
          <div class="form-group">
            <label>PHONE NUMBER (Send Money)</label>
    ...
    function testMpesa() {{
      ...
      const name = "{mpesa_name}".toUpperCase();
      ...
      // Check name
      if (!msgUpper.includes(name)) {{
        errors.push('❌ Receiver name <b>{mpesa_name}</b> not found in SMS');
      }} else {{
        details.name = '✅ Name matched: <b>{mpesa_name}</b>';
      }}
