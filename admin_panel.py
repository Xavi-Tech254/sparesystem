...
<div class="sidebar">
  ...
  <div class="nav">
    ...
    <a href="/admin/sms-inbox" class="{{ 'active' if page=='sms' }}">📱 SMS Inbox</a>
...
def receive_sms():
    """
    SMS Forwarder webhook — receives M-Pesa SMS forwarded from your phone.
    Set this URL in your SMS Forwarder app:
    https://sparesystem.onrender.com/sms/receive
    With header: X-SMS-Key: (your SMS_WEBHOOK_KEY env var)
    """
    # Optional security key check
    sms_key = os.environ.get("SMS_WEBHOOK_KEY", "")
    if sms_key:
        provided = request.headers.get("X-SMS-Key", "") or request.args.get("key", "")
        if provided != sms_key:
            return jsonify({"error": "Unauthorized"}), 401

    data = request.get_json(silent=True) or request.form.to_dict()
    sms_text = data.get("sms") or data.get("message") or data.get("body") or data.get("text") or ""
    ...
    txn_match = re.search(r'\b([A-Z0-9]{10})\b', sms_text)
    if not txn_match:
        txn_match = re.search(r'\b([A-Z0-9]{8,12})\b', sms_text)
    txn_code = txn_match.group(1) if txn_match else None

    # Extract amount
    amt_match = re.search(r'[Kk][Ss][Hh]\.?\s*([\d,]+\.?\d*)', sms_text)
    amount = float(amt_match.group(1).replace(",", "")) if amt_match else 0

    # Extract receiver number
    num_match = re.search(r'(07\d{8}|2547\d{8}|01\d{8})', sms_text)
    receiver_number = num_match.group(1) if num_match else ""

    # Extract receiver name (text between "sent to" and the phone number)
    name_match = re.search(r'sent to\s+([A-Z][A-Z\s]+?)\s+(?:07|2547|01)', sms_text, re.IGNORECASE)
    receiver_name = name_match.group(1).strip() if name_match else ""

    if not txn_code:
        return jsonify({"status": "ignored", "reason": "No transaction code found"}), 200

    # Only store if it looks like an M-Pesa confirmation
    if "confirmed" not in sms_text.lower() and "ksh" not in sms_text.lower():
        return jsonify({"status": "ignored", "reason": "Not an M-Pesa confirmation SMS"}), 200

    try:
        conn = get_db()
        conn.execute(
            "INSERT OR REPLACE INTO received_mpesa (txn_code, amount, receiver_number, receiver_name, raw_sms, received_at) VALUES (?,?,?,?,?,?)",
            (txn_code, amount, receiver_number, receiver_name, sms_text, datetime.now().isoformat())
        )
        conn.commit()
        conn.close()
        return jsonify({"status": "ok", "txn_code": txn_code, "amount": amount}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500


@app.route("/admin/sms-inbox")
@login_required
def sms_inbox_view():
    """View received M-Pesa SMS messages."""
    conn = get_db()
    try:
        messages = conn.execute(
            "SELECT * FROM received_mpesa ORDER BY received_at DESC LIMIT 100"
        ).fetchall()
        messages = [dict(zip([d[0] for d in conn.execute("SELECT * FROM received_mpesa LIMIT 0").description or []], [])) for _ in []] or \
                   [{"txn_code": r[0], "amount": r[1], "receiver_number": r[2], "receiver_name": r[3], "raw_sms": r[4], "received_at": r[5]} for r in messages]
    except:
        messages = []
    conn.close()
    ...
    for m in messages:
        rows += f"""<tr>
            <td><code>{m.get('txn_code','')}</code></td>
            <td>KSh {m.get('amount',0):,.0f}</td>
            <td>{m.get('receiver_number','')}</td>
            <td>{m.get('receiver_name','')}</td>
            <td style="font-size:11px;color:#888">{m.get('received_at','')[:16]}</td>
        </tr>"""

    content = f"""
    <div class="page-title">📱 M-Pesa SMS Inbox</div>
    <div class="page-sub">Real M-Pesa confirmations forwarded from your phone</div>
    <div class="card">
      <div class="card-header">
        <span>Setup Instructions</span>
      </div>
      <div style="padding:16px;font-size:13px;color:#aaa;line-height:1.8">
        <b style="color:#fff">Step 1:</b> Install <b>SMS Forwarder</b> or <b>SMS to URL Forwarder</b> from Play Store<br>
        <b style="color:#fff">Step 2:</b> Set forwarding URL to:<br>
        <code style="background:#111;padding:4px 8px;border-radius:4px;color:#7c6fff">https://sparesystem.onrender.com/sms/receive</code><br>
        <b style="color:#fff">Step 3:</b> Set filter keyword: <b>confirmed</b> (to only forward M-Pesa SMS)<br>
        <b style="color:#fff">Step 4:</b> Set header: <b>X-SMS-Key</b> = your SMS_WEBHOOK_KEY env var<br>
        <b style="color:#fff">Step 5:</b> Go to Admin → Settings → Enable SMS Inbox Verification<br>
      </div>
    </div>
    <div class="card">
      <div class="card-header"><span>Received SMS ({len(messages)} total)</span></div>
      <table>
        <tr><th>TXN CODE</th><th>AMOUNT</th><th>NUMBER</th><th>NAME</th><th>TIME</th></tr>
        {rows if rows else '<tr><td colspan="5" style="text-align:center;color:#555;padding:20px">No SMS received yet</td></tr>'}
      </table>
    </div>"""
    return render_page(content, "sms")
...
def bot_settings():
    ...
    if request.method == "POST":
        ...
        if action == "change_password":
            ...
            sms_verify  = "1" if request.form.get("sms_verify_enabled") else "0"
            b_shop      = request.form.get("banner_shop", "").strip()
            b_dl        = request.form.get("banner_downloader", "").strip()
            b_ai        = request.form.get("banner_ai", "").strip()
            b_account   = request.form.get("banner_account", "").strip()
            b_contact   = request.form.get("banner_contact", "").strip()
            b_payment   = request.form.get("banner_payment", "").strip()
            b_services  = request.form.get("banner_services", "").strip()
            b_links     = request.form.get("banner_links", "").strip()
            b_about     = request.form.get("banner_about", "").strip()
            b_group     = request.form.get("banner_group", "").strip()
            b_rate      = request.form.get("banner_rate", "").strip()
            for key, val in [
                ("bot_banner_image", banner), ("admin_logo", logo),
                ("group_chat_link", group), ("quotes_enabled", quotes),
                ("sms_verify_enabled", sms_verify),
                ("banner_shop", b_shop), ("banner_downloader", b_dl),
                ("banner_ai", b_ai), ("banner_account", b_account),
                ("banner_contact", b_contact), ("banner_payment", b_payment),
                ("banner_services", b_services), ("banner_links", b_links),
                ("banner_about", b_about), ("banner_group", b_group),
                ("banner_rate", b_rate),
            ]:
    ...
    <div class="card">
      ...
      <form method="POST">
        ...
        <div class="form-group">
          <label>🏠 MAIN MENU BANNER IMAGE URL</label>
          <input name="bot_banner_image" value="{banner_img}" placeholder="https://i.postimg.cc/your-image.jpg">
          <small style="color:var(--muted);font-size:11px">Default banner used when no section banner is set. Upload to postimg.cc and paste URL.</small>
        </div>
        <div class="form-group">
          <label>🛍 SHOP BANNER IMAGE URL</label>
          <input name="banner_shop" value="{banner_shop}" placeholder="https://i.postimg.cc/shop-banner.jpg">
        </div>
        <div class="form-group">
          <label>⬇️ DOWNLOADER BANNER IMAGE URL</label>
          <input name="banner_downloader" value="{banner_downloader}" placeholder="https://i.postimg.cc/downloader-banner.jpg">
        </div>
        <div class="form-group">
          <label>🤖 AI ASSISTANT BANNER IMAGE URL</label>
          <input name="banner_ai" value="{banner_ai}" placeholder="https://i.postimg.cc/ai-banner.jpg">
        </div>
        <div class="form-group">
          <label>👤 MY ACCOUNT BANNER IMAGE URL</label>
          <input name="banner_account" value="{banner_account}" placeholder="https://i.postimg.cc/account-banner.jpg">
        </div>
        <div class="form-group">
          <label>📞 CONTACT BANNER IMAGE URL</label>
          <input name="banner_contact" value="{banner_contact}" placeholder="https://i.postimg.cc/contact-banner.jpg">
        </div>
        <div class="form-group">
          <label>💳 PAYMENT BANNER IMAGE URL</label>
          <input name="banner_payment" value="{banner_payment}" placeholder="https://i.postimg.cc/payment-banner.jpg">
        </div>
        <div class="form-group">
          <label>🛠 SERVICES BANNER IMAGE URL</label>
          <input name="banner_services" value="{banner_services}" placeholder="https://i.postimg.cc/services-banner.jpg">
        </div>
        <div class="form-group">
          <label>🔗 LINKS BANNER IMAGE URL</label>
          <input name="banner_links" value="{banner_links}" placeholder="https://i.postimg.cc/links-banner.jpg">
        </div>
        <div class="form-group">
          <label>ℹ️ ABOUT BANNER IMAGE URL</label>
          <input name="banner_about" value="{banner_about}" placeholder="https://i.postimg.cc/about-banner.jpg">
        </div>
        <div class="form-group">
          <label>💬 GROUP CHAT BANNER IMAGE URL</label>
          <input name="banner_group" value="{banner_group}" placeholder="https://i.postimg.cc/group-banner.jpg">
        </div>
        <div class="form-group">
          <label>⭐ RATE US BANNER IMAGE URL</label>
          <input name="banner_rate" value="{banner_rate}" placeholder="https://i.postimg.cc/rate-banner.jpg">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
            <input type="checkbox" name="sms_verify_enabled" {sms_verify_checked}> 
            ENABLE SMS INBOX VERIFICATION
          </label>
          <small style="color:var(--muted);font-size:11px">Requires SMS Forwarder app. See <a href="/admin/sms-inbox" style="color:#7c6fff">SMS Inbox</a> for setup instructions.</small>
        </div>
        <button type="submit" class="btn btn-primary">💾 Save Branding</button>
      </form>
    </div>""".format(
        banner_img=settings.get("bot_banner_image",""),
        banner_shop=settings.get("banner_shop",""),
        banner_downloader=settings.get("banner_downloader",""),
        banner_ai=settings.get("banner_ai",""),
        banner_account=settings.get("banner_account",""),
        banner_contact=settings.get("banner_contact",""),
        banner_payment=settings.get("banner_payment",""),
        banner_services=settings.get("banner_services",""),
        banner_links=settings.get("banner_links",""),
        banner_about=settings.get("banner_about",""),
        banner_group=settings.get("banner_group",""),
        banner_rate=settings.get("banner_rate",""),
